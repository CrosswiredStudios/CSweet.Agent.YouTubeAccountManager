...
- Target .NET 10 and `CSweet.Agent.SDK` 3.4.0. Keep one concurrent job until every provider mutation is proven idempotent.
...
- Before release run `dotnet test CSweet.Agent.YouTubeAccountManager.slnx` and validate the root manifest with SDK 3.4.0.
